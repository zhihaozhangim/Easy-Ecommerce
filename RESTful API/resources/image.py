...
from flask_uploads import UploadNotAllowed  # Error
...
class Image(Resource):
    ...
    def delete(cls, filename: str):
        ...
        try:
            os.remove(image_helper.get_path(filename, folder=folder))   # delete
            return {"message": gettext("image_deleted").format(filename)}, 200
        except FileNotFoundError:
            return {"message": gettext("image_not_found").format(filename)}, 404
        except:  # no permission or file delete for some reason
            traceback.print_exc()
